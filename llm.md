...
***OnTracker Project
What This Is
OnTracker is a Python client that consumes the Doubtfire/OnTrack API to produce a prioritised daily task brief, delivered as an HTML email.

Two operation modes
Mode	Entry point	Auth mechanism	Scheduler
Web app	app.py (Flask + APScheduler)	Bookmarklet → /api/auth/access-token → token stored in SQLite	APScheduler CronTrigger (weekday, configurable hour)
CLI	brief.py	ontrack-cli stored credentials (load_auth_config)	crontab installed by scheduler.py

Bookmarklet token flow
The bookmarklet JS (generated by _bookmarklet() in app.py) runs on the OnTrack page while the user is logged in:
1. POSTs to /api/auth/access-token with delete_auth_token: false and credentials: include to exchange the active session for a fresh auth token.
2. Falls back to scanning localStorage / sessionStorage for any key containing "token" if the fetch fails.
3. Redirects to the OnTracker setup page with base_url, username, and auth_token as query params.

API endpoints OnTracker calls
Endpoint	Purpose	File
GET /api/unit_roles	Token validation	fetcher.py → validate_token
GET /api/projects	Fetch active projects (filtered by unit end_date >= today)	fetcher.py → fetch_active_projects_direct
GET /api/projects/:id	Fetch tasks + task definitions for a project	fetcher.py → fetch_tasks_direct
GET /api/projects/:project_id/task_def_id/:task_def_id/comments	Latest tutor feedback comment	fetcher.py → fetch_last_feedback_direct
GET /api/units/:unit_id/task_definitions/:id/task_pdf.json	Task sheet PDF (CLI mode)	fetcher.py → fetch_task_sheet
GET /api/projects/:project_id/task_def_id/:task_def_id/submission	Latest submission PDF (CLI mode)	fetcher.py → fetch_submission

Request headers
{"Username": "<username>", "Auth-Token": "<token>", "Accept": "application/json"}

Data model OnTracker cares about (from API responses)
Field path	Used for
project.id	Scoping task/comment fetches
project.unit.code	Section header labels in the brief
project.unit.end_date	Filtering out ended units
project.user.id	Identifying student comments to skip in feedback fetch
task.task_definition_id	Linking to comments and definition metadata
task.status	Routing into urgent/todo/waiting/submitted/done buckets
task.due_date / task_definition.target_date	Deadline display and priority sort
task.target_grade	Grade badge (P/C/D/HD) and scoring weight
task.completion_date	"Recently Completed" window filter
task.abbreviation	Direct link: {base_url}/projects/{project_id}/dashboard/{abbreviation}

Priority scoring (builder.py → _score)
Urgency tier (lower = higher priority): time_exceeded → redo_submission / fix_and_resubmit → need_help → everything else.
Within a tier, tasks are sorted by days_left adjusted for grade: HD tasks get a 21-day bonus (treated as more urgent than an equivalent Pass task with the same deadline).

SQLite schema (db.py)
users(id, base_url, username, auth_token, email, brief_hour, created_at)
One row per subscriber. upsert on email conflict — re-subscribing updates the token and schedule hour.

***Doubtfire API — Reference
What This Is
