...
from core.fetcher import validate_token
...
def main() -> None:
    ...
    log.info("Started at %s — polling every %ds", started.strftime("%H:%M:%S"), args.interval)
    ...
    try:
        while True:
            ...
            if fresh != token:
                rotations += 1
                log.info("Check #%d [+%s] — valid, token ROTATED (rotation #%d)  ...%s -> ...%s",
                         checks, elapsed_str, rotations, token[-8:], fresh[-8:])
                token = fresh
            else:
                log.info("Check #%d [+%s] — valid, token unchanged  ...%s",
                         checks, elapsed_str, token[-8:])
